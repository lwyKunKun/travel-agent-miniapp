...
# ============ 异步任务模型 (小程序端使用: 创建任务 → 轮询进度/结果) ============

class TaskCreatedResponse(BaseModel):
    """任务创建响应"""
    success: bool = Field(default=True, description="是否成功")
    task_id: str = Field(..., description="任务ID, 用于轮询查询")
...
class TaskStatusResponse(BaseModel):
    ...
    stage: str = Field(default="", description="当前阶段描述, 如: 正在搜索景点…")
    progress: int = Field(default=0, ge=0, le=100, description="进度百分比 0~100")
    created_at: str = Field(default="", description="任务创建时间 ISO8601")
    finished_at: Optional[str] = Field(default=None, description="任务完成时间 ISO8601")
    error: Optional[str] = Field(default=None, description="失败原因 (status=failed 时返回)")
    data: Optional[TripPlan] = Field(default=None, description="行程结果 (status=completed 时返回)")
